...
namespace MonkeyWrench.Web.UI
{
	...
	public partial class GetMetadata : System.Web.UI.Page
	{
		...
		bool hasMetadata(DBWorkView2 w)
		{
			...
			if (w.summary == null || !w.summary.Contains("metadata"))
				return false;
		...
		bool isValidRevision(WebServiceLogin login, List<DBRevisionWork> revisionWorkList)
		{
			...
			Func<DBRevisionWork, bool> validRevisions = r =>
				Utils.LocalWebService.GetViewLaneData(login, r.lane_id, "", r.host_id, "", r.revision_id, "").WorkViews.Any(hasMetadata);
