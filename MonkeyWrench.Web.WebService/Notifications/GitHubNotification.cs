...
namespace MonkeyWrench.WebServices {
	public class GitHubNotification : NotificationBase {
		...
		/**
		 * Struct containing needed info about a RevisionWork that can be returned in one query.
		 */
		...
		/**
		 * Given a RevisionWork ID and Work ID, retrieves the derived info needed
		 * to build the status message from the database.
		 */
		...
		/**
		 * Given a GitHub repository URL (ex. `git@github.com:user/repo.git`),
		...
		private Tuple<string,string> gitRepoToGitHub(string repoUrl) {
			...
			if (!m.Success)
				throw new ArgumentException ("Invalid GitHub repository: " + repoUrl);
			return Tuple.Create (m.Groups [1].Value, m.Groups [2].Value);
		}

		/**
		 * Builds an HttpWebRequest to the statuses API.
		 * Sets up the URL and headers.
		 */
		...
		/**
		 * Builds a JSON object to post to the statuses API.
		 */
		...
		/**
		 * Sends the request with the passed data and checks the response for errors.
		 */
		...
		// Not used, but still needs an implementation.
		...
		{
			var req = buildRequest (info.repoURL, info.hash);
			var description = String.Format ("Lane: {0}, Host: {1}, Scheduled.", info.lane, info.host);
