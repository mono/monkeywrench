...
namespace MonkeyWrench.WebServices
{
	public static class Notifications
	{
		...
		static readonly Dictionary<int, List<NotificationBase>> notifications_per_lane = new Dictionary<int, List<NotificationBase>> ();

		public static void Start ()
		{
			List<DBLaneNotification> lane_notifications;
					...
					using (IDataReader reader = cmd.ExecuteReader ()) {
						while (reader.Read ()) {
							...
							if (n.ircidentity_id.HasValue) {
								Logger.Log ("Starting irc notification {0} ({1})", n.id, n.name);
								notifications.Add (new IrcNotification (n));
							} else if (n.emailidentity_id.HasValue) {
								Logger.Log ("Starting email notification {0} ({1})", n.id, n.name);
								notifications.Add (new EMailNotification (n));
							} else if (n.githubidentity_id.HasValue) {
								Logger.Log ("Starting GitHub notification {0} ({1})", n.id, n.name);
								notifications.Add (new GitHubNotification (n));
							} else {
								throw new ApplicationException ("Unknown notification type (id: "+n.id+")");
							}
						}
						reader.NextResult ();

						lane_notifications = DBRecord.LoadMany<DBLaneNotification> (reader);
					}
				}
			...
			using (DB db = new DB ()) {
				...
				using (IDbCommand cmd = db.CreateCommand ()) {
					cmd.CommandText = @"
						...
						WHERE RevisionWork.lane_id = @lane_id AND RevisionWork.host_id = @host_id AND RevisionWork.completed
							AND Revision.date > (SELECT date FROM Revision WHERE id = @revision_id) AND
							(RevisionWork.state = 3 OR (@nonfatal OR RevisionWork.state = 8))
					...
					DB.CreateParameter (cmd, "command_id", work.command_id);
					DB.CreateParameter (cmd, "nonfatal", nonfatal);
